...
RSpec.describe Legion::Extensions::CognitiveArchaeology::Helpers::Artifact do
  ...
  describe '#decay!' do
    it('decreases') {
      p0 = art.preservation
      art.decay!
      expect(art.preservation).to be < p0
    }
    it('clamps 0') {
      50.times { art.decay!(rate: 0.1) }
      expect(art.preservation).to eq(0.0)
    }
  end
  describe '#restore!' do
    it('increases') {
    ...
    it('clamps 1') {
      10.times { art.restore!(boost: 0.2) }
      expect(art.preservation).to eq(1.0)
    }
  end
  it('fragment?') {
    art.preservation = 0.1
    expect(art).to be_fragment
  }
  it('well_preserved?') {
    art.preservation = 0.9
    expect(art).to be_well_preserved
  }
  it('label') { expect(art.preservation_label).to be_a(Symbol) }
  it('link_to') {
    art.link_to('x')
    art.link_to('x')
    expect(art.contextual_links.size).to eq(1)
  }
