...
RSpec.describe Legion::Extensions::CognitiveArchaeology::Helpers::Artifact do
  ...
  describe '#initialize' do
    ...
    it 'defaults preservation to DEFAULT_PRESERVATION' do
      expect(build.preservation).to eq(Legion::Extensions::CognitiveArchaeology::Helpers::Constants::DEFAULT_PRESERVATION)
    end
    ...
    Legion::Extensions::CognitiveArchaeology::Helpers::Constants::ARTIFACT_TYPES.each do |t|
      it "accepts artifact type :#{t}" do
        expect { build(type: t) }.not_to raise_error
      end
    end

    Legion::Extensions::CognitiveArchaeology::Helpers::Constants::DOMAIN_TYPES.each do |d|
      it "accepts domain :#{d}" do
        expect { build(domain: d) }.not_to raise_error
      end
    end

    Legion::Extensions::CognitiveArchaeology::Helpers::Constants::EXCAVATION_DEPTH_LEVELS.each do |level|
  ...
  describe '#decay!' do
    it 'decreases preservation by PRESERVATION_DECAY' do
      ...
      expect(artifact.preservation).to be_within(0.001).of(original - Legion::Extensions::CognitiveArchaeology::Helpers::Constants::PRESERVATION_DECAY)
    end
